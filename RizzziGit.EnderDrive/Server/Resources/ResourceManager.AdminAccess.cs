...
public record class AdminAccess : ResourceData
{
    ...
    public UnlockedAdminAccess WithAesKey(byte[] aesKey)
    {
        return new()
        {
            Id = Id,
    ...
    public UnlockedAdminAccess Unlock(UnlockedUserAuthentication userAuthentication)
    {
...
public sealed partial class ResourceManager
{
    ...
    )
    {
        ...
        await Insert(transaction, [adminAccess]);

        return adminAccess.WithAesKey(KeyManager.Encrypt(user, adminKey.AesKey));
    }
    ...
    public IQueryable<AdminAccess> GetAdminAccesses(
        ...
        Query<AdminAccess>(
            ...
            (query) =>
                query.Where(
                    (item) =>
                        (userId == null || item.UserId == userId) && (id == null || item.Id == id)
                )
        );
}
