...
public record class AdminKey : ResourceData
{
    ...
    public UnlockedAdminKey Unlock(string password)
    {
        using Rfc2898DeriveBytes rfc2898DeriveBytes =
            new(Encoding.UTF8.GetBytes(password), Salt, Iterations, HashAlgorithmName.SHA256);
        byte[] aesKey = rfc2898DeriveBytes.GetBytes(32);

        byte[] decryptedChallengeBytes = KeyManager.Decrypt(aesKey, EncryptedChallengeBytes);
...
public sealed record class UnlockedAdminKey : AdminKey
{
    public static implicit operator byte[](UnlockedAdminKey file) => file.AesKey;
...
public sealed partial class ResourceManager
{
    ...
    )
    {
        ...
        await Insert(transaction, [adminKey]);

        return new()
        {
            Id = adminKey.Id,
            Iterations = adminKey.Iterations,
            Salt = adminKey.Salt,

            ChallengeBytes = adminKey.ChallengeBytes,
            EncryptedChallengeBytes = adminKey.EncryptedChallengeBytes,
    ...
    public async Task<AdminKey?> GetExistingAdminKey(ResourceTransaction transaction)
    {
        return await Query<AdminKey>(transaction, (query) => query)
            .ToAsyncEnumerable()
