<script lang="ts">
	...
	const icon: IconOptions = { icon: 'plus', thickness: 'solid' };
...
{#if $isDesktop}
	...
	<AdminSidePanel name="Create" {icon}>
		<div class="panel">
			<p>Create and display news banners to users upon login.</p>

			<Button onclick={onopen} {foreground}>
				<p>Create</p>
			</Button>
		</div>
	</AdminSidePanel>
...
{#if $isMobile}
	<AppButton label="Create News" onclick={onopen} {icon} />
{/if}
