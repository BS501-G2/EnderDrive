...
export type FileBrowserResolve =
		...
		| [
				type: FileBrowserResolveType.File,
				fileId:
					| string
					| null
		  ]
...
export type FileBrowserContext =
	ReturnType<
		typeof createFileBrowserContext
	>['context'];

export function useFileBrowserContext() {
	return getContext<FileBrowserContext>(
		contextName
	);
}
...
export function createFileBrowserContext(
	...
	const showDetails =
		persisted(
			'file-browser-config-show-details',
			false
		);
	const isLoading =
		writable(
	...
	const context =
		setContext(
			contextName,
			{
				...
				setFileListContext:
					(
						context: FileBrowserListContext
					) => {
						fileListContext.set(
							context
						);
				...
				pushRefresh:
					(
						...
						if (
							...
							throw new Error(
								'Refresh callback is already registered'
							);
						}
