...
export type LandingPageEntry = { id: number; name: string; content: Snippet; showButton: boolean };
...
export function createLandingContext() {
	...
	const context = setContext<LandingContext>(landingContextName, {
		pushLandingEntry: (name: string, content: Snippet, showButton: boolean) => {
			...
			pages.update((pageButtons) => [...pageButtons, { id, name, content, showButton }]);
		...
		pushFooter: (name, content) => {
			...
			footer.update((footer) => [...footer, { id, name, content }]);
	...
	return { context, pages, footer, buttons, login };
}
