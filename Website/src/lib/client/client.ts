...
export type ClientContext =
  ReturnType<
    typeof createClientContext
  >['context'];

const requestSymbol: unique symbol =
  Symbol(
    'Request'
  );
const requestFunctionsSymbol: unique symbol =
  Symbol(
    'Request Functions'
  );
        ...
        (
          ...
          switch (
                ...
                send(
                  {
                    ...
                    id: packet.id,
                    reason:
                      InternalErrorReason.DuplicateId
                  }
                );
                break;
              }
            ...
            case PacketType.ResponseCancel: {
              ...
              reject(
                new ClientResponseCancelledError()
              );
              break;
            }
            ...
            case PacketType.ResponseInternalError: {
              ...
              reject(
                new ClientInternalResponseError(
                  packet.reason
                )
              );
              break;
            ...
            notificationContext.update(
              (
                ...
                if (
                  ...
                  throw new Error(
                    'Notification context is already set'
                  );
                }
...
export function useClientContext() {
  return getContext<ClientContext>(
    clientContextName
  );
}
...
export class ClientResponseError extends ClientError {
  public constructor(
    ...
    super(
      `Server responded with code: ${ResponseCode[responseCode]}`
    );
...
export class ClientInternalResponseError extends Error {
  public constructor(
    ...
    super(
      `Remote returned an internal error`,
      options
    );
...
export interface FileSnapshotResource
  extends ResourceData {
  createTime: Date;
  fileId: string;
  fileContentId: string;
...
export interface FileVirusReportResource
  extends ResourceData {
  fileId: string;
  fileContentId: string;
  fileSnapshotId: string;
